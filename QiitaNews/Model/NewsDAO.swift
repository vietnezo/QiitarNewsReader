...
class NewsDAO {
    ...
    public func insert(item: T) -> Bool {
        ...
        let insert = table.insert(newsId <- item.newsId, userId <- item.userId, title <- item.title, content <- item.content, updatedAt <- item.updatedAt)
    ...
    public func getAll() -> [T] {
        ...
        for item in items {
            results.append(
                News(id: item[id], newsId: item[newsId], title: item[title], userId: item[userId], content: item[content], updatedAt: item[updatedAt])
            )
        }
